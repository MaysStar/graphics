...
model AssetLoadModel(char* FolderPath, char* Name)
{
    ...
    char* FilePath = CombineString(FolderPath, Name);
    ...
    if (!Scene || Scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE || !Scene->mRootNode)
    {
        ...
        MessageBoxA(NULL, Error, "Importer Error", MB_OK | MB_ICONERROR);
        InvalidCodePath;
    }
    ...
    for (u32 MaterialId = 0; MaterialId < Scene->mNumMaterials; ++MaterialId)
    {
        ...
        if (CurrMaterial->GetTextureCount(aiTextureType_DIFFUSE) > 0)
        {
            ...
            char* TexturePath = CombineString(FolderPath, (char*)TextureName.C_Str());

            u32* UnFlippedTexels = (u32*)stbi_load(TexturePath, &CurrTexture->Width, &CurrTexture->Height, &NumChanels, 4);
            ...
            for (u32 Y = 0; Y < CurrTexture->Height; ++Y)
            {
                ...
                {
                    u32 TexelId = Y * CurrTexture->Width + X;

                    CurrTexture->Texels[TexelId] = UnFlippedTexels[(CurrTexture->Height - Y - 1) * CurrTexture->Width + X];
                }
            }
    ...
    //MessageBoxA(NULL, "No errors.", "Importer Info", MB_OK | MB_ICONINFORMATION);
    ...
    for (u32 MeshId = 0; MeshId < Result.NumMeshes; ++MeshId)
    {
        ...
        for (u32 VertexId = 0; VertexId < DstMesh->VertexCount; ++VertexId)
        {
            ...
            {
                CurrVertex->Uv = V2(SrcMesh->mTextureCoords[0][VertexId].x, SrcMesh->mTextureCoords[0][VertexId].y);
            }
            else
            {
                CurrVertex->Uv = V2(0.0f);
            }
        }
