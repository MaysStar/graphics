...
void ClipPolygonToAxis(clip_result* Input, clip_result* Output, clip_axis Axis)
{
	...
	for (u32 TriangleId = 0; TriangleId < Input->NumTriangles; TriangleId++)
	{
		...
		switch (NumBehindPlane)
		{
			case 0:
			{
				// Triangle is absolutlly visible
				u32 CurrTrianglesId = Output->NumTriangles++;
				Output->Vertices[3 * CurrTrianglesId + 0] = Vertices[0];
				Output->Vertices[3 * CurrTrianglesId + 1] = Vertices[1];
				Output->Vertices[3 * CurrTrianglesId + 2] = Vertices[2];
			}break;
			...
			case 1:
			{
				...
				for (u32 EdgeId = 0; EdgeId < 3; EdgeId++)
				{
					...
					if (!StartBehindPlane)
					{
						Output->Vertices[3 * CurrTrianglesId + CurrVertexId++] = StartVertex;
					}
					...
					if (!isTriangleAdded && CurrVertexId == 3)
					{
						...
						CurrTrianglesId += 1;
						CurrVertexId = 0;
						Output->Vertices[3 * CurrTrianglesId + CurrVertexId++] = Output->Vertices[3 * (CurrTrianglesId - 1) + 0];
						Output->Vertices[3 * CurrTrianglesId + CurrVertexId++] = Output->Vertices[3 * (CurrTrianglesId - 1) + 2];
					}

					if (StartBehindPlane != EndBehindPlane)
					{
						Output->Vertices[3 * CurrTrianglesId + CurrVertexId++] = CalculateIntersection(StartVertex, EndVertex, Axis);
					}
